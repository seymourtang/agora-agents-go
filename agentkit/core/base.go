...
func ValidateProfileProviders(profile Profile, properties map[string]interface{}, preset string) error {
	...
	categories := []struct {
		...
		{name: "asr", config: AsMap(properties["asr"])},
		{name: "llm", config: AsMap(properties["llm"])},
		{name: "tts", config: AsMap(properties["tts"])},
		{name: "mllm", config: AsMap(properties["mllm"])},
		{name: "avatar", config: AsMap(properties["avatar"])},
	}

	for _, category := range categories {
		if len(category.config) == 0 {
			continue
		}
		vendor, _ := category.config["vendor"].(string)
		if vendor == "" && category.name == "llm" {
			vendor = "openai"
		}
		if err := validateProfileVendor(profile, category.name, vendor); err != nil {
			return err
		}
	}
	...
	if profile == ProfileCN {
		if preset != "" {
			for _, item := range splitPresetList(preset) {
				if !isCNPreset(item) {
					return fmt.Errorf("preset %q is not supported by the CN agent profile", item)
				}
			}
		}
		return nil
	...
	if profile == ProfileGlobal && preset != "" {
		for _, item := range splitPresetList(preset) {
			if isCNPreset(item) {
				return fmt.Errorf("preset %q is only supported by the CN agent profile", item)
			}
		}
	}
...
func validateProfileVendor(profile Profile, category string, vendor string) error {
	if category == "mllm" && profile == ProfileCN {
		return fmt.Errorf("MLLM is not supported by the CN agent profile")
	}
	if vendor == "" {
		return nil
	}

	cnVendors := map[string]map[string]bool{
		"asr": {
			"fengming":       true,
			"tencent":        true,
			"microsoft":      true,
	...
	if profile == ProfileCN {
		if allowed := cnVendors[category]; allowed != nil && !allowed[vendor] {
			return fmt.Errorf("%s vendor %q is not supported by the CN agent profile", category, vendor)
		}
		return nil
	}

	if allowed := cnVendors[category]; allowed != nil && allowed[vendor] {
		return fmt.Errorf("%s vendor %q is only supported by the CN agent profile", category, vendor)
	}
	return nil
}
...
func isCNPreset(preset string) bool {
	switch preset {
	default:
		return false
	}
